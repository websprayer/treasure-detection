...
with Blocks(
    ...
    description_inf = """
    <p style="text-align: center;">
        <span style="font-size: 28px; font-weight: bold;">图像识别</span>
        <br>
        上传图片或者视频以识别treasure 
    </p>
    """
    ...
    with gr.Tab('预测'):
        ...
        with gr.Row():
            with gr.Column():
                model_path = gr.Dropdown(value = 'train', label = '模型路径', choices = sorted(os.listdir('runs/detect/')))
                refresh_inf = gr.Button(value = '点击刷新模型列表')
                inf_model_type = gr.Radio(value = 'YOLOv8n', label = "模型类型", choices = ['YOLOv8n', 'YOLOv8s', 'YOLOv8m', 'YOLOv8l', 'YOLOv8x'])
            with gr.Column():   
                select_inp = gr.Radio(value = 'Image', choices = ['Image', 'Video'], label = '选择输入类型')
                url = gr.Textbox(label = '图像或视频的URL', interactive = True, visible = True)
                img = gr.Image(label = '上传图片', interactive = True)
                vid = gr.Video(label = 'Input video', interactive = True, visible = False)
        with gr.Row():
            inf_btn = gr.Button(value = 'treasure识别')
        with gr.Row():
            outybox = gr.Textbox(label = '输出 (metrics, boxes, etc.)')
            with gr.Column():
